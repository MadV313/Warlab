...
async def countdown_ephemeral(base_msg: str, followup: discord.webhook.WebhookMessage):
    """
    Send a 23-second countdown message that is **ephemeral** and does NOT block
    the raid logic.
    """
    try:
        wait_msg = await followup.send(f"{base_msg} *(23s)*", ephemeral=True)
    ...
    async def attack_phase(self, interaction: discord.Interaction):
        ...
        if self.phase < 3:
            ...
            async def countdown_ephemeral(base_msg, followup):
                try:
                    wait_msg = await followup.send(content=f"{base_msg} *(23s)*", ephemeral=True)
                    for seconds in range(22, 0, -1):
                        await asyncio.sleep(1)
                        try:
